...
def main():
    ...
    # A. Generate Pool of Peak Clips (T2V)
    print(f"[Generator] Creating a pool of {args.num_peak_clips} unique peak clips...")
    peak_pool_segments = []
    for i in range(args.num_peak_clips):
        peak_pool_segments.append({
            'index': i,
            'duration': args.peak_len 
        })
            
    peak_pool_paths = generator.generate_peak_clips_batch(peak_pool_segments, batch_size=args.batch_size)
    peak_pool_paths.sort(key=lambda p: int(p.stem.split('_')[1]))
